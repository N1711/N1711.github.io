...
var textures = {
	texture0: 'img/wood-68.jpg',
	texture1: 'img/twxt2.jpg',
	texture2: 'img/txwt3.jpg',
	texture3: 'img/twxt1.png'
}
...
function event22() {																				//I have no clue why I named this function
	...
	clc.addEventListener("click", loadDeskC);
	var clh = document.querySelector('#desk_d');
	clh.addEventListener("click", loadDeskD);
	var cld = document.querySelector('#desk_b');
	cld.addEventListener("click", loadDesk);
	var cle = document.querySelector('#white');
	cle.addEventListener("click", loadTex.bind(null, textures.texture2));
	var cld = document.querySelector('#oak');
	cld.addEventListener("click", loadTex.bind(null, textures.texture0));
	var clf = document.querySelector('#ash');
	clf.addEventListener("click", loadTex.bind(null, textures.texture1));
	var clg = document.querySelector('#wood');
	clg.addEventListener("click", loadTex.bind(null, textures.texture3));
}
...
const loadTex = (tex) => {
	if(typeof myObj != "undefined") {
		...
		var texture = textureLoader.load(tex);
		texture.flipY = false;
		myObj.traverse ( ( o ) => {
			if ( o.isMesh ) {
